...
- The model starts from the prompt and generates one token after another. 模型从提示词开始，一个 token 一个 token 地生成。
- At each step, earlier tokens can influence the next token. 每一步生成时，前面的 token 都可能影响下一个 token。
...
- What does it mean to say that an AI model predicts the next token? “预测下一个 token”是什么意思？
- Why might earlier words influence later words? 为什么前面的词会影响后面的词？
- Is this visualization the same as an explanation? Why or why not? 这个可视化等于解释模型吗？为什么？
- What does this tool help us understand, and what does it still hide? 这个工具帮助我们理解了什么？又仍然隐藏了什么？
